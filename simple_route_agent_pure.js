...
/**
 * Initial/default state for simpleRouteAgent.
 */
...
/**
 * Pure function version of SimpleRouteAgent. No pub/sub, no mutation.
 *
 * @param {typeof DEFAULT_STATE} state - Previous state (use DEFAULT_STATE on first call)
 * @param {object} inputs
 * @param {number} inputs.time - Current time
 * @param {{x:number,y:number}} [inputs.position] - Current position from odometry (optional)
 * @param {boolean} [inputs.waypointReached] - True if the current waypoint was reached (optional)
 * @param {{waypoints:{x:number,y:number}[]}} [inputs.routeUpdate] - New route (optional)
 *
 * @returns {{ state: typeof DEFAULT_STATE, targetWaypoint: {x:number,y:number}|null, logJson: object|null }}
 */
export function simpleRouteAgent(state, { time, position, waypointReached, routeUpdate }) {
    let s = { ...state }
    ...
    return { state: s, targetWaypoint, logJson }
}
